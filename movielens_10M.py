...
def _read_raw_data():
    ...
    file_path = path + '/dataset_10M/'
    return (open(file_path + 'ra.train').read().split('\n'),
			open(file_path + 'ra.test').read().split('\n'),
			open(file_path + 'movies.dat').read().split('\n'),
			open(file_path + 'u.genre').read().split('\n'))
...
def fetch_movielens_10M(data_home=None, indicator_features=True, genre_features=False,
